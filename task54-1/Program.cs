...
void SortedRowArray (int[,] array)
{
  ...
  for (int row = 0; row < array.GetLength(0); row++)
  {
    ...
    {
      rowArray[col] = array[row, col]; // создаю одномерный массив из значений строки
      indexArray++;
    }
    Array.Sort(rowArray);
    Array.Reverse(rowArray);
    // добавляю массив из строки во временный одномерный масссив
    Array.Copy(rowArray,0, tempArray, indexArray-array.GetLength(1), array.GetLength(1));
  }
  indexArray = 0;
  for (int i =0; i < array.GetLength(0); i++)
  {
    ...
    {
      array[i, j] = tempArray[indexArray];
      indexArray++;
    }
  }
